...
<div class="mt-3 grid items-end gap-2 sm:grid-cols-2 lg:grid-cols-3">
    <div>
        <label for="{{ $list }}-{{ $i }}-annuityAmount" class="text-xs text-gray-600">Amount to annuitise (£)</label>
    ...
    <div>
        ...
        <input id="{{ $list }}-{{ $i }}-annuityAtAge" type="number" wire:model="{{ $list }}.{{ $i }}.annuityAtAge" class="{{ $field }}">
    ...
    <div>
        ...
        <input id="{{ $list }}-{{ $i }}-annuityIncomeFromAge" type="number" wire:model="{{ $list }}.{{ $i }}.annuityIncomeFromAge" class="{{ $field }}">
    ...
    <div>
        ...
        <input id="{{ $list }}-{{ $i }}-annuityRate" type="text" inputmode="decimal" wire:model="{{ $list }}.{{ $i }}.annuityRate" class="{{ $field }}" placeholder="7.2">
        @error($list.'.'.$i.'.annuityRate') <p class="mt-1 text-xs text-red-700">{{ $message }}</p> @enderror
        <p class="mt-1 text-xs text-gray-500">Use a real quote for your age and health; ~7.2% is a rough level joint-life-at-65 guide.</p>
    </div>
    <div>
        <label for="{{ $list }}-{{ $i }}-annuityEscalation" class="text-xs text-gray-600">Increases</label>
        <select id="{{ $list }}-{{ $i }}-annuityEscalation" wire:model="{{ $list }}.{{ $i }}.annuityEscalation" class="{{ $field }}">
    ...
    <div class="lg:col-span-3">
        ...
        @if ($row['annuityJoint'] ?? false)
            <div class="mt-1 max-w-xs">
                ...
                <input id="{{ $list }}-{{ $i }}-annuitySurvivorFraction" type="text" inputmode="decimal" wire:model="{{ $list }}.{{ $i }}.annuitySurvivorFraction" class="{{ $field }}" placeholder="50">
