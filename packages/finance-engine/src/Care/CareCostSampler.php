 ...
 * {@see CareAssumptions}. Per person: a Bernoulli draw on the sex-differentiated probability of
 * ever needing care (women's rate is materially higher — see CareAssumptions); if it hits, a
 * duration (exponential with the assumed mean, rounded to whole years, floored at 1 and capped)
 * and a residential-vs-nursing draw fix the annual cost. The spell is placed at the end of life
 * (the final $duration years up to the sampled age at death), because care need concentrates near
 * death — so it depends on the death ages already sampled for the path.
 *
...
final class CareCostSampler
{
    ...
    /**
     * @param  list<array{id: string, sex: Sex, currentAge: int, deathAge: int}>  $people
    ...
    public function sampleHousehold(array $people, Randomizer $rng): array
    {
        ...
        foreach ($people as $person) {
            // One Bernoulli per person (always drawn, so the RNG stream is stable), against this
            // person's sex-differentiated care probability, then the duration + type draws only
            // when care actually occurs.
            if ($rng->nextFloat() >= $this->assumptions->probabilityOfCare($person['sex'])) {
                continue;
            }
