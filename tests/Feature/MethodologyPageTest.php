...
/**
 * The public /methodology page renders docs/spec/METHODOLOGY.md — the single source that is also the
...
final class MethodologyPageTest extends TestCase
{
    ...
    {
        $this->assertContains('spec/METHODOLOGY.md', (array) config('assistant.methodology_docs'),
            'METHODOLOGY.md must stay in config(assistant.methodology_docs) so the assistant can retrieve it.');
        $this->assertFileExists(base_path('docs/spec/METHODOLOGY.md'));
    }
}
