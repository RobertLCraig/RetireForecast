...
| running_costs | Money 🔒 | pence/yr | yes | maintenance + insurance. Council tax moved OUT of this bucket on 2026-09-06 (see below) |
| annual_council_tax | Money 🔒 | pence/yr | yes | **2026-09-06** (card 0047). The household's council tax bill, held apart from `running_costs` because it is the one running cost that SHRINKS: the 25% single-person discount, pension-age Council Tax Reduction and the disabled band reduction all act on it, and none of them could reach a bundled figure. `Benefits\CouncilTax` owns the three reliefs and the order they apply in; `YearResult::councilTax()` reports what was charged. Unlike the rest of `running_costs` it is NOT scaled by `ownership_share`: council tax is charged to whoever LIVES in the dwelling. Null = still inside `running_costs`, charged in full for life, which every scenario stored before this is, and which the `council_tax_bundled` input note says on the screen. Builder key `property.councilTax` |
| disabled_band_reduction | CouncilTaxBand | | yes | **2026-09-06** (card 0047). Null = not claimed. Set to the band the dwelling is actually in, which is what says the disabled band reduction applies: the bill is then charged as the band below (`CouncilTaxBand::reducedNinths()`, band A falling to 5/9 of band D since it has no band below). Holding the band here rather than beside a yes/no flag makes "the reduction applies, but from which band?" unrepresentable. NOT means-tested. Builder key `property.councilTaxDisabledBand` |
