...
2. **The first `scenarios:audit` after that will report existing completed runs** as carrying no
   integrity stamp (they predate the column). That is the honest reading — an unstamped result
   cannot be vouched for — and it clears as soon as those scenarios are re-run, which the head of
