...
2. **Say whether the interest-only product replaces the stored base**, by picking a numbered option
   below. Pass: the answer written into `## Decided`. An agent cannot settle this: it turns on whether
   Francis and Chee would accept a plan that is fundable but still leaves the survivor short, which is
   their call, not a modelling result. Compare them here:
   <http://retireforecast.test/scenarios/9/results> · <http://retireforecast.test/scenarios/55/results>
   · <http://retireforecast.test/scenarios/49/results>
...
It does not work. Modelled as scenarios 55–58, the product buys one year and does not carry the
survivor: the interest alone is **~102%** of Chee's net income, against 142% on the LiveMore
repayment. Sell-and-buy-cheaper (49) still never runs short and still ends highest of the keep/sell
set at £300,741. The full figures, the arithmetic check and what is not modelled are in the private
doc.
...
4. **Accept that no keep-the-flat route survives and make 49 (sell & buy cheaper £165k) the base**,
   ...
   times now, but it is a life decision, not a modelling one, and it forecloses the flat.

## Recommendation
**Option 1 until the reversion rates arrive, then re-decide between 2 and 4.** The revert rate is
worth more than the headline rate here: at 7.24% the interest-only product fails in 2035, a year
*before* the base it was meant to improve on. Do not promote a base on a five-year fix whose sixth
year is unknown.
