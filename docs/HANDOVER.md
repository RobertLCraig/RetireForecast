...
_Last updated: 2026-08-23 (card **0007** built the last two slices of withdrawal sequencing: an ad-hoc "fill the bands" pension draw is now taken **UFPLS-style** (a quarter tax-free while the Lump Sum Allowance lasts) instead of being taxed on 100% of the gross; tax-free cash **crystallises** the rest of the pot so no pound gets a second quarter; **flexible access caps later money-purchase contributions at the MPAA** under every draw order, disclosed as an assumed figure in the year it starts; and the **optimiser** runs all three named orders and reports the cheapest with its saving, as the difference of two of the engine's own runs. An inherited pot is excluded from all of it. **`ScenarioForecaster::ENGINE_VERSION` is now `finance-engine/pcls-crystallisation`** — figures moved, so a run stored before this card must not be read beside one stored after. Deferred and carded: 0073 to 0080. Nothing here has been looked at in a browser. Previously, card 0016 closed the adviser-parity remainder that had acceptance criteria: the engine now **uses** the ISA allowance as well as enforcing it (bed-and-ISA, on by default, disclosed as an assumed figure and switchable via `ForecastSettings::$useIsaAllowance`, so stored-scenario figures have moved), contributions are capped by the annual allowance as well as the MPAA, and a non-earner can pay £2,880 for £3,600 of pot; A4 salary sacrifice, B3 estate checklist and B4 annual review were listed as tasks on that card but carry no acceptance criterion and are not built; card 0015 closed two of its six flagged v1 refinements, a resident's Pension Credit now counting into the care charge and CGT deemed-occupation absences now entered as such rather than by hand; the other four stay open on that card, three of them wanting a sourced modelling figure the repo does not hold; card 0014 export-all-to-PDF is queued and built one forecast at a time past eight of them, arriving as a zip of one PDF each; card 0013 nominal-pounds toggle on the time-series charts, reading the projector's own pre-deflation year; cards 0011 capacity for loss and 0012 release blockers, the latter partly: nonce CSP and the guidance-only posture done, and a11y now clean on every page a machine can reach with the unautomatable WCAG 2.2 criteria settled bar a human pass; the stress-test data licence and that human pass are still open, and the comparison still should not be read off until the ranking-movers at the head of the queue are fixed)_
