...
/**
 * Assembles the printable report for a scenario — the data behind the PDF export.
 *
 * It lives outside the controller because two callers need it: the HTTP download in
 * App\Http\Controllers\ScenarioPdfController, and the queued export ({@see ScenarioExport}),
 * which renders one scenario at a time on the worker. One assembly, one home, so a queued
 * export can never print different figures from a direct download.
 */
class ScenarioReport
{
    /**
     * Every scenario the "export all" print covers, in export order: ready bases
     * newest-first (the dashboard's order), each followed by its ready what-if children.
     * Drafts have nothing to print and are excluded, as on the single download.
     *
     * This is also what the export is SIZED by, so the count that decides batching and
     * the list that is printed can never disagree.
     *
     * @return Collection<int, Scenario>
     */
    public function scenarios(User $user): Collection
    {
     ...
     *
     * Note this holds EVERY report in memory at once, which is why it is only used for a
     * small export; the queued export walks {@see scenarios()} one at a time instead.
     *
     * @return list<array<string, mixed>>
     */
    public function reports(User $user): array
    {
        return $this->scenarios($user)
