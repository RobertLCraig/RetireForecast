...
final class ThresholdCsvExporter
{
    ...
    public static function rows(ThresholdResult $run): array
    {
        ...
        // The full swept grid with each point's Monte Carlo confidence interval.
        $rows[] = ['Lever value', 'Success probability', 'CI low', 'CI high', 'Paths'];
        foreach ($curve->points as $point) {
            $rows[] = [
                self::leverValue($point->leverValue),
