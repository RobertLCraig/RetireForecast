...
- **Finding + specced (not built) — IHT is collected-but-unconsumed; relationship status blocked on it (2026-07-04):** the `ihtModelled` toggle is stored + shown in what-if diffs/GDPR but **no forecast reads it** — `InheritanceTaxCalculator` (complete + tested) is never called in the pipeline, so turning IHT on changes nothing (a silent drop). Married-vs-cohabiting materially changes IHT + survivor treatment but can't be expressed (the engine implicitly assumes married). The unit of work — **wire IHT into the forecast, relationship-status aware** — is fully specced for a fresh agent in **[docs/PLAN-iht-and-relationship-status.md](docs/PLAN-iht-and-relationship-status.md)** (calculator done; only wiring + an `EstateValuer` + a `relationshipStatus` input + a results panel remain). Rob's call: spec, don't build this session.
