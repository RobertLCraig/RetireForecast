...
**Still to build:** **Phase D go-live polish** — a11y CI (axe/Pa11y), CSP header, PDF export,
...
Carlo perf item is DONE (suite 332 green).
What remains in Tier-2: a11y CI (axe/Pa11y), CSP header, PDF export, 2FA enrolment UI.** Items 1–6 below are kept only to record that they
...
- **Tier 2 — Phase 5–6:** ✅ **demo preset/seeder DONE (2026-06-28)** (`App\Demo\DemoScenario` +
  ...
  13.9 s → 8.9 s → 4.75 s, byte-identical); still open: PDF export, **CSP header** (none set, charts are embedded),
  a11y CI (axe/Pa11y), 2FA enrolment UI. Scotland config pack is deliberately out of v1.
...
php artisan test                            # everything: expect 332 passed (2780 assertions)
...
is the only snapshot). This session's commits (newest first): **Phase D Tier-2 — 10k-path Monte Carlo perf**
(`IncomeTaxCalculator::totalPence()` lean integer twin sharing `bandedTax()` with `compute()` + projector routing +
`IncomeTaxTotalPenceTest` 1,120-cell equivalence grid + worker-JIT docs; this commit); **Phase D Tier-2 — demo preset/seeder**
