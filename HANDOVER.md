...
- **Known bugs / broken (from the 2026-06-28 re-review — detail in docs/PLAN.md "Review findings"):** **[MED]** the **PDF Monte-Carlo summary can diverge from the screen** — `ScenarioPdfController` picks the latest *Done* run while the results page shows the latest run only if it is Done, so after a Done run followed by a cancelled/failed one the PDF still prints the earlier MC table the screen is hiding; the PDF MC section also carries **no run provenance** (mode/paths/seed/date), so a 1,000-path preview could print as the report (introduced with the PDF this session). **[MED]** **`ForecastSettings::$freezeEndYear` is documented but never implemented** — its docblock says thresholds resume rising in 2031 but `PathProjector` keeps them frozen for the whole projection (the v1 limit below), so the docblock contradicts the behaviour and long forecasts overstate post-2031 drag; decision pending (implement vs correct the docblock). **[LOW]** GIA disposal basis split rounds two float-derived parts independently (penny drift in a tax calc, `PathProjector:533-534`); **[LOW]** `medianDepletionYear` reaches only the screen headline (not the comparison table / CSV / PDF); **[LOW]** no cash-interest conservation test mirroring the GIA one. None block the green suite. Documented v1 scope limits (all flagged in code): income tax is England/Wales/NI only (Scotland throws); emergency tax models the over-deduction magnitude, not PAYE-table pennies; mortality grid ages 50–100 / years 2025–2074 with clamping + a non-ONS tail above 100 (cap 110); forecast now taxes GIA dividends + cash interest annually AND realises CGT on GIA disposal (A5 complete; ISA tax-free; GIA/cash grow at capital only; v1 omits capital-loss relief + judges the CGT band on non-savings income); tax thresholds held frozen for the whole projection; DB escalation + triple lock as smooth growth factors; buy-vs-rent takes main-home CGT as £0 (PRR) and no SDLT surcharge; house/salary growth deterministic inside the Monte Carlo.
...
Carlo perf, the CSP/security-headers item, the 2FA enrolment UI, PDF export and the a11y CI scaffold are ALL DONE (suite 353 green).
...
- [ ] **Re-review findings (2026-06-28) — decisions/fixes pending (detail in docs/PLAN.md "Review findings").**
  - **[MED] PDF Monte-Carlo summary divergence + missing provenance.** Decision needed: when the newest run
    failed/cancelled, should the screen **and** PDF fall back to the **last successful run** (recommended), or should
    the PDF just match the screen and hide it too? Either way, stamp the PDF MC section with mode/n_paths/seed/date so
    a preview can't print as the report. Then fix `ScenarioPdfController::monteCarloSummary` to match the chosen
    selection + add a test.
  - **[MED] `freezeEndYear` documented but not implemented.** Decision needed: **implement** threshold un-freezing
    from `ForecastSettings::$freezeEndYear` (accuracy gain for long forecasts; recommended), or **correct the
    docblock** + drop the dead param to match the conservative frozen-for-whole-projection behaviour.
  - **[LOW] batch:** derive the GIA disposal basis split so the two parts can't drift + add a multi-disposal basis
    reconciliation test; surface `medianDepletionYear` in `ResultPresenter::comparison()` so it reaches all four
    surfaces; add a cash-interest conservation test mirroring the GIA one.
...
php artisan test                            # everything: expect 353 passed (2858 assertions)
php artisan test --testsuite=Engine        # engine only: expect 139 passed (1757 assertions)
