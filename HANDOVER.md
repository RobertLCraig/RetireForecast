...
_Last updated: 2026-07-01 (Lane B, from Rob's V2 browser review: **engine correctness fix — a surviving partner now inherits the deceased's assets** (previously stranded → every couple forecast falsely "ran out" at the first death; **re-run stored couple runs**); a **Compare-page "Re-run all N" button** (one full run per plan in the family); **how-to-claim Pension Credit** guidance; and the cashflow ladder now surfaces **investment growth** (capital appreciation) beside investment income. Earlier this session: **single-strategy report** with variations as **what-if scenarios** (Compare), charts moved to the top + **milestone annotations**, a **"Let out & rent elsewhere"** what-if, **let home = assessable capital**, the **buy>proceeds feasibility flag**, and a **tax-free disability-benefit income type**. Remaining Lane-B deferred: stop the bundled mortgage payment after a repay (needs a `while_mortgaged` condition), the in-place forced-sale model. Post-v1 backlog (Lane A) still open: v2 annuitisation, stress-test panel, ONS-refresh script, care-cost assumptions. Feature work pending Rob's browser sign-off. **⚠️ Concurrent agent sessions may be live (lanes A/B/C/D below) — read the next section before you commit.**)_
