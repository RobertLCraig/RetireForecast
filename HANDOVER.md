...
_Last updated: 2026-06-30 (handover tidy: adviser-legibility workstream complete, folded into Current state; What's next now future-only — verification + an optional post-v1 backlog. Latest build: **partial-PRR CGT on a let former home** (gov.uk HS283). All pending Rob's browser sign-off.)_
...
2. **Post-v1 enhancement backlog** (optional, none blocking; full list + rationale in docs/PLAN.md "External review triage"). Highest adviser-value first, mostly from data the engine already computes: a **stress-test panel** (historical sequences 1973–74 / dot-com / GFC / 1970s inflation through the engine); a **longevity-distribution visual** (median / p10 / p90 last-survivor age + P(live past 95) from the joint-life sampler); **what-if sliders** (±retirement age / ±spend / year-1 shock / longevity shock). Then **v2 modelling depth** — an **annuitisation** option (priced off the mortality tables) and **care-cost stochasticity**; **neutral diagnostics** behind the lint (implied withdrawal rate, critical yield, replacement rate, a narrative report); **hardening** (build-time `verified_on` source-freshness check, a tamper-evident hash over each run's snapshot, an annual ONS-refresh script, caching deterministic forecasts by input hash).
