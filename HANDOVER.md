...
_Last updated: 2026-06-30 (post-v1 backlog: **longevity distribution**, **source-freshness check**, and **per-year surplus/shortfall + configurable safety floor** built. Remaining items (stress-test, sliders, v2 annuitisation/care, ONS-refresh, retirement-month proration) are **researched and now gated on data-source/UX/assumptions decisions from Rob** — see What's next #2. Feature work pending Rob's browser sign-off.)_
...
The adviser-legibility workstream is done (see Current state). What remains is verification and an optional post-v1 backlog; nothing is on a blocking critical path.
...
2. **Post-v1 backlog — each now gated on a decision from Rob (researched 2026-06-30; sources in the session log).** (a) **Stress-test panel** — recommended: **historical-sequence backtesting** (run real bad-start sequences 1973–74 / 2000 / 2008 / 1970s-inflation through the engine). Needs Rob to approve the **historical data source** (Barclays Equity Gilt Study or the UBS/DMS Global Investment Returns Yearbook — sourced per the verified_on rule) **and** a new engine entry point to run a *fixed* return/inflation sequence (the deterministic forecaster uses expected returns). (b) **What-if sliders** — needs a quick **UX nod** (proposed: live sliders on the results page for retirement age / spend / return / longevity, re-running the deterministic preview; Rob: "more flexibility, rebuilds okay"). (c) **v2 annuitisation** — model buying a level/RPI, single/joint annuity with part of the pot; needs the **rate basis** (published market rates ≈7.2% level joint at 65, or priced off the engine's mortality + a gilt yield). (d) **v2 care-cost stochasticity** — needs the **assumptions** (P(need care), entry age, duration; self-funder ≈£1,300/wk residential, £1,600/wk nursing, LaingBuisson 2025/26). (e) **ONS-refresh script** — Rob approved; needs the **ONS source/format** to ingest + diff against the current cohort table. (f) **Retirement-month proration** — Rob wants salary to stop mid-year; the engine currently drops the *whole* retirement year (pays while `age < plannedRetirementAge`), so it under-counts that year. Needs a retirement-*month* input + prorating. Low-value hardening leftovers (confirm worth it): tamper-evident run hash, forecast caching. The academic "neutral diagnostics" idea is **dropped** — replaced by the per-year surplus/shortfall + safety-floor view (built, see Current state).
