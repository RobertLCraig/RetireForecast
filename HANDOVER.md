...
_Last updated: 2026-06-28 (Phase D Tier-1 data-integrity — displayed-figure provenance: fan/ladder CSVs reproduce the panel tables figure-for-figure + interpretation shares one `%` formatter; built on the same day's forecast-boundary reconciliation invariants: HousingProceeds single-source net-proceeds + total-wealth-from-parts, which caught + fixed a 1-pence PathProjector round-of-sum drift; suite 309 green. Earlier: admin-panel lockdown — canAccessPanel() gated on a new is_admin flag, first admin via `php artisan user:make-admin`; suite 298 green. Earlier today: gov.uk figure-verification pass complete (every ⚠️ figure re-confirmed + stamped verified_on 2026-06-27, no value changed, pensions-in-IHT now enacted); A5 complete. See newest session-log entries). Earlier notes:_
...
- **Tier 1 — trust (blocks "shown as real"):** ✅ **the gov.uk verification pass is DONE (2026-06-27).** Every
  ...
  fixed) **and the displayed-figure provenance test is built** (panel == CSV == interpretation, one shared %
  formatter); what's left is only the user-facing import reconciliation panel. See DECISIONS 2026-06-27.
...
- [~] **Data-layer integrity guardrails — importer + forecast-boundary + provenance BUILT; only the user-facing reconciliation panel remains.**
  ...
  **Displayed-figure provenance now BUILT (2026-06-28):** `DisplayedFigureProvenanceTest` proves the fan CSV and the
  ...
  comparison table, CSV exports and `Interpretation` all share (one figure, one home). **Only remaining Tier-1
  data-integrity item:** an import reconciliation panel surfaced to the user (the import + forecast-boundary +
  provenance test layers are done). **Caveat:** the IWT fixture was built from a *masked* dump of the real
...
php artisan test                            # everything: expect 309 passed (1589 assertions)
