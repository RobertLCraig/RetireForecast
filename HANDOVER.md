...
**Stage:** active
**Status:** Rebuild through **C4 done**; now in **Phase D (trust + go-live)**. **A5 complete** — GIA dividends + cash interest taxed annually (asset grows at capital only, conservation tested) **and CGT realised on GIA disposal** (pro-rata gain vs cost basis, shared £3k AEA, 18/24% by band; CGT-incidence tested). GIA/CGT no longer deferred. Suite **293 green** (engine 129 / app 164). **Next: the rest of Phase D** — the gov.uk ⚠️ figure-verification pass (incl. the new PLSA + `investmentIncomeYield` + CGT/AEA figures), then go-live polish (a11y CI, CSP header, panel lockdown, perf, PDF, 2FA UI). _The narrative that follows is the full build history; the live picture is the **▶ REBUILD** callout below + the newest session-log entry._
...
_Last updated: 2026-06-27 (Phase D — A5 complete: GIA dividends + cash interest taxed annually AND CGT realised on GIA disposal. See newest session-log entries). Earlier notes:_
...
**Still to build:** **Phase D (continued)** — gov.uk ⚠️ figure-verification pass (incl. the new PLSA,
`investmentIncomeYield`, and CGT-rate/£3k-AEA figures A5 relies on) + go-live polish (a11y CI, CSP, panel
lockdown, perf, PDF, 2FA UI). Full plan: docs/PLAN.md "Sector-informed build plan".
