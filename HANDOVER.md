...
**Stage:** active
**Status:** Phase D go-live. The **adviser-legibility workstream** is the active priority (from Rob's 2026-06-29 browser walkthrough). Its legibility **presentation layer is built** — house-sale waterfall, assumptions panel, itemised per-year spend, life-event milestones, input-sanity notes (all on the results page, pending Rob's browser sign-off). Rob's browser pass then set a clear **new direction**: make **all thresholds/assumptions user-editable** in the UI (nothing hardcoded) and turn **buy-vs-rent into a deliberate what-if/Compare**. The **contingent-cost correctness fix (#1, option b) is now built** — expense lines auto-classified by label with variant-aware charging, so the sell variants no longer pay a phantom mortgage and the commute stops at retirement (the per-line *override UI* is still to build). **The next build is the per-variant cashflow ladder**, which *shows* the corrected per-strategy numbers. See What's next + docs/PLAN.md + DECISIONS 2026-06-29 + docs/RESEARCH-editable-assumptions-ux.md.
_Last updated: 2026-06-29 (built **#1 contingent-cost fix**, option b — sell variants drop the phantom mortgage/service-charge, commute stops at retirement, auto-classified by label; engine + assembler, reconciliation-tested. Next: the per-variant deterministic ladder using `HousingComparison::variantInputs()`. Everything-editable direction in DECISIONS + docs/RESEARCH-editable-assumptions-ux.md.)_
