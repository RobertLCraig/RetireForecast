...
**Stage:** active
**Status:** Rebuild through **C4 done**; now in **Phase D (trust + go-live)**. **A5 income side done** (GIA dividends + cash interest taxed annually; asset grows at capital only — conservation tested); **A5 CGT-on-disposal is in progress**. Suite **291 green** (engine 127 / app 164). **Next: A5.3 (CGT on GIA disposal)** then A5.4 (surface on results + docs); then the rest of Phase D — gov.uk ⚠️ figure-verification pass (incl. the new PLSA + `investmentIncomeYield` figures) and go-live polish (a11y CI, CSP header, panel lockdown, perf, PDF, 2FA UI). _The narrative that follows is the full build history; the live picture is the **▶ REBUILD** callout below + the newest session-log entry._
...
_Last updated: 2026-06-27 (Phase D started — A5 income side: GIA dividends + cash interest taxed annually, asset grows at capital only; CGT-on-disposal in progress. See newest session-log entry). Earlier notes:_
...
**Still to build:** **Phase D** — gov.uk ⚠️ figure-verification pass (incl. the new PLSA figures) + the
deferred GIA/CGT modelling (A5) + go-live polish (a11y CI, CSP, panel lockdown, perf, PDF, 2FA UI). Full plan:
docs/PLAN.md "Sector-informed build plan".
...
└─ packages/finance-engine
   ...
   └─ tests/{Money,Tax,Pension,StatePension,Property,Benefits,Iht,Care,Dto,
             Assumptions,Benchmark,Mortality,Forecast,MonteCarlo,Housing,
...
php artisan test                            # everything: expect 287 passed (1144 assertions)
php artisan test --testsuite=Engine        # engine only: expect 123 passed (589 assertions)
