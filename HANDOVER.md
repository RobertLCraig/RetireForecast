...
_Last updated: 2026-07-03 (**all five silent-drop backlog fixes done** — the collected-but-unconsumed class the
2026-07-02 audit found is closed: (1) `DbPension::spousePensionFraction` — survivor keeps `pension × fraction`;
(2) `Person::salaryGrowth` — per-person real salary-growth override; (3) **DB commutation** —
`commutationLumpSum`/`commutationFactor` (retyped `Percent`→`float`) reduce the pension + pay the tax-free lump sum
at NRA; (4) **`Person::niCategory`** — category-aware employee NI (sourced B/E/I reduced, D/J/L/Z deferred, C/K/S/X
nil); (5) **`Property::ownershipShare`** — tenants-in-common beneficial share scales wealth, means-test, IHT +
sale proceeds/CGT (researched HMRC convention). Completeness tests pin each; DECISIONS 2026-07-02/07-03.)_
