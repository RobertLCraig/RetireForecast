...
_Last updated: 2026-07-05 (**decision-support Phase 4 lever menu STARTED** — the first survivor lever, the
**survivor's-DB-share** sweep. New engine `Sweep\Lever\SurvivorDbFractionLever` (+ immutable
`DbPension::withSpousePensionFraction`) sweeps a DB scheme's survivor fraction 0–100% — monotone `Increasing`,
CRN-safe (touches neither mortality nor returns), and it varies only schemes that already offer a survivor pension
(never invents one). Wired through `LeverKey::SurvivorDbFraction` + `buildLever` + `defaultGrid` + the presenter
arms; `ThresholdExplorer` offers it only for a couple with a DB survivor pension. **Four survivor levers remain**
(per-person longevity split, defer-the-survivor's-SP, joint-life annuity survivor %, care on/off pinned — the
modelling-heavy pair last; see What's next). Earlier the same day: the **survivor-cliff story** —
`ResultPresenter::incomeFloor()` gained a **`survivor`** twin at the deepest survivor year (reconciling to the
ladder's survivor rows via the extracted one-definition `floorAt()`) + the signed coverage **`cliff`**, a factual
before/after **dumbbell** (+ PDF line); and **Phase 3 COMPLETE** (the combination-comparison surface on Compare).
DECISIONS 2026-07-05. Phases 1 + 2 (the queued `ThresholdResult` backend + the "How far can we go?" panel) are in
the Session log.)_
