...
**Still to build:** **Phase D go-live polish** — a11y CI (axe/Pa11y), PDF export. Full plan: docs/PLAN.md "Sector-informed build plan".
...
routes/web.php                                         # /, /welcome (ack), /dashboard, /scenarios/{create,edit,child,compare,results}, /account/{export,security,destroy}
...
Carlo perf item is DONE, the CSP/security-headers item is DONE, and the 2FA enrolment UI is DONE (suite 348 green).
What remains in Tier-2: a11y CI (axe/Pa11y), PDF export.** Items 1–6 below are kept only to record that they
...
**Independent of the rebuild (Phase 2 step 5 + go-live):** a11y CI (axe/Pa11y), PDF export
(✅ demo preset/seeder DONE 2026-06-28; ✅ 10k perf DONE 2026-06-28; ✅ CSP header DONE 2026-06-28;
✅ 2FA enrolment UI DONE 2026-06-28; ✅ `User::canAccessPanel()` lockdown DONE 2026-06-27;
...
- **Tier 2 — Phase 5–6:** ✅ **demo preset/seeder DONE (2026-06-28)** (`App\Demo\DemoScenario` +
  ...
  trait + challenge/confirm-password screens; residual = browser eyeball that the QR scans); still open: PDF export,
  a11y CI (axe/Pa11y). Scotland config pack is deliberately out of v1.
- **Tier 3 — shipped-surface gaps:** ✅ **2FA enrolment UI DONE (2026-06-28)** (was: Fortify 2FA on, no screens),
  real-browser ApexCharts verification (only the tables/text are tested), assumption-set figures not numerically
  editable in Filament. ✅ **`User::canAccessPanel()` lockdown DONE 2026-06-27** — gated on `is_admin` (the `interpret`
...
php artisan test                            # everything: expect 348 passed (2845 assertions)
...
is the only snapshot). This session's commits (newest first): **Phase D Tier-2 — 2FA enrolment UI**
(`App\Livewire\AccountSecurity` + `TwoFactorAuthenticatable` trait + challenge/confirm-password screens + `/account/security`
route behind `password.confirm` + nav link + `TwoFactorAuthenticationTest`; this commit); **Phase D Tier-2 — security headers / CSP**
