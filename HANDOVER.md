...
_Last updated: 2026-07-02 (**first silent-drop backlog fix**: `DbPension::spousePensionFraction` now paid — the
survivor keeps `pension × fraction` for life instead of £0 on the member's death; new `survivorDbIncomeNominal`
mirrors the annuity survivor income, `SurvivorDbPensionTest` pins it, builder hint added. DECISIONS 2026-07-02
(survivor DB pension). Next silent-drop fix: `Person::salaryGrowth`.)_
