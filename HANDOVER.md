...
_Last updated: 2026-07-05 (**decision-support Phase 4 lever menu — two survivor levers built**. Newest: the
**joint-life-annuity survivor share** sweep (`Sweep\Lever\SurvivorAnnuityFractionLever` +
`AnnuityPurchase::withSurvivorFraction` / `DcPension::withAnnuityPurchase`), the annuity twin of the DB lever — sweeps
`survivorFraction` 0–100% on annuities that are already joint-life, monotone `Increasing` + CRN-safe, gated to a
couple with a joint-life annuity. It follows the **survivor's-DB-share** sweep (`SurvivorDbFractionLever` +
`DbPension::withSpousePensionFraction`), the same pattern on a DB scheme. Both wired through `LeverKey` +
`buildLever`/`defaultGrid` + the presenter arms; both vary only an *existing* survivor benefit, never inventing one.
**Three survivor levers remain** (per-person longevity split, defer-the-survivor's-SP, care on/off pinned — the first
two desync RNG → `LeverDirection::Unknown`; see What's next). Earlier the same day: the **survivor-cliff story**
(`ResultPresenter::incomeFloor()` survivor-year twin + before/after dumbbell) and **Phase 3 COMPLETE** (the
combination-comparison surface on Compare). DECISIONS 2026-07-05. Phases 1 + 2 (the queued `ThresholdResult` backend +
the "How far can we go?" panel) are in the Session log.)_
...
| docs/PLAN-decision-support.md | Staged plan for the "lever thresholds + combination comparison" feature. **Phases 0–3 BUILT + Phase 4 PART-BUILT** (Phase 0 spine 2026-07-04; Phases 1–3 + the Phase-4 survivor-cliff story, 2026-07-05). Q2 (family-money × PC) + Q3 (target) resolved. **Remaining:** the Phase-4 survivor **lever menu**, then Phase 5 (2-D frontier) + Phase 6 (assistant tie-in). Hardened by a 4-agent review. |
