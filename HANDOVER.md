...
_Last updated: 2026-07-05 (**decision-support Phase 1 COMPLETE** — the queued threshold backend on top of the
already-built compute core: a `ThresholdResult` model/migration that is BOTH the run (status/progress/cancel, reusing
`SimulationStatus`) AND the store (curve + crossing via `ThresholdOutcomeMapper`); `App\DecisionSupport\ThresholdRunner`
+ the `RunLeverThreshold` job mirroring the `SimulationRun` triad; **two-layer staleness** (delete-on-edit like runs +
an **inputs-hash cache key** off the effective builder-state); an owner-scoped **CSV** carrying the shared, extracted
`App\Export\ExportDisclaimer`; provenance (seed/paths/grid/versions/assumption snapshot) frozen per record. **Next:
Phase 2 UI.** DECISIONS 2026-07-05. The prior 2026-07-04 IHT + decision-support Phase 0/compute-core work is in the
Session log.)_
