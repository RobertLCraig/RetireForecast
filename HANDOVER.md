...
**built its Phase 1** (a grounded, local scenario-explainer on the results page; docs/RESEARCH-local-assistant.md).
...
- **Done — local-model assistant, Phase 1 (grounded scenario-explainer) (2026-07-03):** an in-page "chatbot" (`App\Livewire\ScenarioAssistant` on the results page) that explains **this** forecast in plain English on a **local** model (Ollama). App-layer only, engine untouched. `App\Assistant\`: `OllamaChatClient` behind a `ChatClient` interface (local HTTP, fails loudly via `AssistantUnavailable`, strips `<think>`); `ScenarioContext` — the engine figure snapshot that is BOTH the model's context and the grounding allow-list; `FigureGrounding` (**G1** — refuses any £/year/% figure not engine-derived), reuses `App\Compliance\OutputPhrasing` (**G2** — recommendation phrasing blocked in guidance-only mode, allowed in advice mode via the `interpret` gate); `AssistantService` orchestrates with one corrective retry then refuses rather than show a bad answer. **Inert by default** (`config('assistant.enabled')`, env `ASSISTANT_ENABLED`). 19 unit tests incl. the "never surfaces an invented figure" invariant; **end-to-end verified against the real `qwen3:14b`** (grounded answers pass; it declines to invent). v1 = synchronous call with a "Thinking…" state; richer context / streaming / a side-nav entry / a Livewire feature test are fast-follows. **Phases 2 (methodology doc-RAG) + 3 (backlog capture — the model's only write) remain specced.** See DECISIONS 2026-07-03 + docs/RESEARCH-local-assistant.md. **Pending Rob's browser sign-off** (enable the flag, open a results page).
...
- [ ] **In-app local-model assistant (2026-07-03)** — **Phase 1 BUILT** (grounded scenario-explainer, see Current state, pending Rob's browser sign-off); **Phases 2–3 specced.** docs/RESEARCH-local-assistant.md + docs/PLAN.md "In-app local-model assistant" + DECISIONS 2026-07-03. A local (Ollama) results-page "chatbot" that **explains + captures, never predicts/calculates/builds**: phased (1) ✅ grounded scenario-explainer, (2) methodology doc-RAG, (3) research/feature capture. **The model's only write (Phase 3) is `queueBacklogItem` (append-only, attributed); it never builds code or offers to.** Two guardrails — G1 figure-grounding, G2 runtime phrasing partition — the latter a flagged public-release blocker.
