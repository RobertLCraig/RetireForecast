...
_Last updated: 2026-06-30 (the **builder now highlights a what-if's changed inputs** — each input whose value differs from the base is ringed in amber, via a server-computed changed-path set + a bundled `builder-diff.js` matching inputs by `wire:model`. Recent prior work: **one-click quick what-ifs**, **what-ifs highlight what they changed from the base**, and the **editable-assumptions core**. All pending Rob's browser sign-off. Next: live preview + the longevity-lever UX.)_
...
- **Done — the builder highlights a what-if's changed inputs (2026-06-30):** when editing a what-if, each input whose value differs from the base plan is **ringed in amber** (plus a one-line "fields you change from the base are highlighted" banner). `ScenarioBuilder::changedFromBasePaths()` computes the changed form-state leaf paths (a **positional** diff of the live `builderState()` vs the base's `effectiveBuilderState()`, **index-based** to match each input's `wire:model`), renders them on the form (`data-builder-diff` + `data-changed-paths`), and a bundled, CSP-safe, morph-aware `resources/js/builder-diff.js` rings each matching input via a plain `.builder-diff-changed` class. One server set + one script covers all ~70 inputs uniformly (no per-input markup). Pure progressive enhancement (the form works without JS). On opening an existing what-if the changed fields highlight immediately. Livewire-tested (childMode flags the changed path + renders the hook; a base does neither). **Pending Rob's browser sign-off.**
...
the changed fields highlight on load; live-as-you-type follows the deferred `wire:model` round-trips. Livewire-tested
(childMode flags `housing.annualRent` + renders the hook; a base does neither). Suite green; assets rebuilt.
