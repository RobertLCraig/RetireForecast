...
_Last updated: 2026-06-30 (adviser-legibility complete; post-v1 backlog started — **longevity distribution** + **source-freshness check** built. Stopped: every remaining backlog item now needs Rob's direction (see What's next + the latest session-log entry). Feature work pending Rob's browser sign-off.)_
...
2. **Post-v1 enhancement backlog** (optional, none blocking; full list + rationale in docs/PLAN.md "External review triage"). Highest adviser-value first, mostly from data the engine already computes: a **stress-test panel** (historical sequences 1973–74 / dot-com / GFC / 1970s inflation through the engine); **what-if sliders** (±retirement age / ±spend / year-1 shock / longevity shock). (The **longevity-distribution visual** is now built — see Current state.) Then **v2 modelling depth** — an **annuitisation** option (priced off the mortality tables) and **care-cost stochasticity**; **neutral diagnostics** behind the lint (implied withdrawal rate, critical yield, replacement rate, a narrative report); **hardening** (a tamper-evident hash over each run's snapshot, an annual ONS-refresh script, caching deterministic forecasts by input hash — the `verified_on` source-freshness check of this list is now built, see Current state).
