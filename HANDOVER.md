...
- **In progress:** nothing mid-edit; tree clean and **all committed**. Phase 2 steps 1–4 done + the above; step 5 (demo/polish) not started.
...
The engine + a working prototype app are built and green (224 tests). The agreed next phase **rebuilds the
scenario + expenditure data shape** to the sector-informed model, then adds the new features. **Read first:
docs/PLAN.md "Sector-informed build plan (2026-06-26)" (full steps + the gotchas table A–P) · DATA-MODEL.md
"Planned shape changes (2026-06-26)" · DECISIONS.md 2026-06-26 (×3) · docs/RESEARCH-cashflow-modelling.md.**
...
1. **Scenario data-shape + Edit a saved forecast.** Persist the builder **form-state** on the scenario
   (encrypted `builder_state`); the engine `Household` DTO becomes a *derived* artifact regenerated on save
   (no reverse-mapper — single source). Edit route `/scenarios/{scenario}/edit`, **owner-scoped**; `save()`
   → update-or-create; **invalidate stale runs/results on edit** (gotcha B).
2. **"Create child" what-ifs + Compare.** A child = a **delta** of overridden form-state paths on a base
...
3. **3-tier line-item budget.** Line items `{id,label,amount(annual),category,savedAsAsset}` as the
...
4. **Drill-down + usable-vs-total wealth.** Deterministic **cashflow ladder** from `YearResult[]` per
   scenario; **split usable/liquid from total (incl. home)** — the engine already has `liquidWealth` /
   `propertyWealth` / `totalWealth` per year, just aggregate terminal **liquid** in the `Simulator` and show
   both (fixes the cards paradox, gotcha P; a stopgap label is already shipped). Per-pension current→projected→income.
5. **Per-person longevity adjustment** (engine: fixed assumed age / ±years / mortality multiplier feeding
   `JointLifeSampler`; golden-master test) — enables the lifespan what-if.
...
php artisan test                            # everything: expect 224 passed (894 assertions)
php artisan test --testsuite=Engine        # engine only: expect 105 passed (520 assertions)
...
| Doc | Purpose |
|-----|---------|
| docs/PLAN.md | The full approved implementation plan. Source of truth for scope, data model, tax rules, Monte Carlo design, phasing. Holds the "Sector-informed build plan (2026-06-26)". |
...
## Session log
_2026-06-26 (REBUILD session — engine enrichment Phase A + results drill-down C3)_ — Fresh session to
...
tagged the prototype **`prototype-v1`** (a8f1f68). Locked the rebuild decisions (DECISIONS 2026-06-26 ×2).
...
_2026-06-26 (builder UX from live use + sector research/planning)_ — Rob ran the app and gave UX
