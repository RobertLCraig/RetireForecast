...
**Cross-lane in-flight — coordinate, do not grab.** Lane B forced-housing deferred: the `while_mortgaged` mortgage-payment-stop is **built** (docs/PLAN-mortgage-payment-stop.md); only the in-place forced-sale model remains, **specced + decisions resolved, ready to build (docs/PLAN-in-place-forced-sale.md)**. Lane C withdrawal-sequencing next steps (docs/PLAN-withdrawal-sequencing.md). Lane D multi-property (docs/PLAN-multi-property.md, DRAFT, [needs Rob]).
