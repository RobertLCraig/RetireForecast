...
_Last updated: 2026-07-01 (Lane B: the report is now **single-strategy**, variations are **specialised what-if scenarios** (Compare); the two charts moved to the top + gained **milestone annotations**; a **"Let out & rent elsewhere"** what-if; a **let home = assessable capital** engine fix; and two V2 deferred refinements — the **buy>proceeds feasibility flag** + a first-class **tax-free disability-benefit income type**. Remaining Lane-B deferred: stop the bundled mortgage payment after a repay (needs a `while_mortgaged` condition), the in-place forced-sale model. Post-v1 backlog (Lane A) still open: v2 annuitisation, stress-test panel (gated on sourced historical data), ONS-refresh script, care-cost assumptions. Feature work pending Rob's browser sign-off. **⚠️ Concurrent agent sessions may be live (lanes A/B/C/D below) — read the next section before you commit.**)_
...
2. **Post-v1 backlog (Rob approved all my recommendations 2026-06-30; sources in the session log).** Built this session: **what-if sliders** (b), **retirement-year salary proration** (f, via birthday — explicit retirement-*month* override still possible). **Remaining:** (a) **Stress-test panel** — historical-sequence backtesting (run real bad-start sequences 1973–74 / 2000 / 2008 / 1970s-inflation through the engine). Needs **authoritative sourced historical return/inflation sequences** (Barclays Equity Gilt Study or UBS/DMS Yearbook — must be real per the no-fabrication / verified_on rule; this is the gate, not the code) **and** a new engine entry point to run a *fixed* sequence (the deterministic forecaster uses expected returns). (c) **v2 annuitisation** — buy a level/RPI single/joint annuity with part of a DC pot at an age: pot − £X at the age, then a lifetime escalating survivor-aware income = £X × rate (rate a **user input defaulted to a sourced ~7.2% level-joint-at-65**, so no fabricated age table); reuses the DB-income machinery + a pot-reduction event in `PathProjector`. (d) **v2 care-cost stochasticity** — needs the **assumptions** (P(need care), entry age, duration; self-funder ≈£1,300/wk residential, £1,600/wk nursing, LaingBuisson 2025/26). (e) **ONS-refresh script** — needs the **ONS source/format** to ingest + diff against the cohort table. Low-value hardening leftovers (confirm worth it): tamper-evident run hash, forecast caching.
