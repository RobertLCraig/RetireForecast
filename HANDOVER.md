...
**Stage:** active
**Status:** Phase D go-live. The **adviser-legibility workstream** is the active priority (from Rob's 2026-06-29 browser walkthrough). Its legibility **presentation layer is built** — house-sale waterfall, assumptions panel, itemised per-year spend, life-event milestones, input-sanity notes (all on the results page, pending Rob's browser sign-off). Rob's browser pass then set a clear **new direction**: make **all thresholds/assumptions user-editable** in the UI (nothing hardcoded), do the **contingent-cost correctness fix via option (b)** (auto-classify expense lines + per-line override), and turn **buy-vs-rent into a deliberate what-if/Compare**. **The next build is #1 contingent costs (option b).** See What's next + docs/PLAN.md + DECISIONS 2026-06-29 + docs/RESEARCH-editable-assumptions-ux.md.
...
1. **Adviser-legibility workstream** — the priority from Rob's 2026-06-29 browser walkthrough (full detail: docs/PLAN.md "Adviser-legibility workstream (2026-06-29)"; decisions: DECISIONS 2026-06-29). None of it is an engine bug (determinism + mortality re-verified); it is cost placement + missing explanation. **Guiding principle (Rob): trust comes from explanation — every headline figure must be traceable on screen to its inputs/assumptions, so this sits above remaining go-live polish.** The **explainer / show-your-working layer is built (2026-06-29, pending Rob's browser sign-off):** the house-sale waterfall (proceeds decomposition + per-option destination, selling-cost rate shown beside the £), the assumptions panel (real-vs-nominal labelled, single-source blended return), itemised per-year spend (essential/discretionary), the **life-event milestones** timeline (when each person retires / their State Pension starts / takes a pension / dies — death from a new single-source engine field `ForecastResult::deathCalendarYears`; the house-sale marker waits for the per-variant ladder), and **input-sanity notes** (a heads-up when an input did something drastic — no salary from a retirement age at/below current age; a death floored to the base year) on the results page — see DECISIONS. Remaining, in order:
   1. **[correctness — the next build] Contingent-cost placement, option (b).** Housing costs (mortgage payment, service charge / ground rent, owner maintenance) and status costs (commute) sit in shared `expenseProfile`, so they're charged in *every* buy-vs-rent variant (phantom mortgage + service charge in *sell & rent* / *buy outright*; commute that never stops at retirement). **Option (b), confirmed by Rob:** auto-classify each expense line by category/label (mortgage / service charge / ground rent → *while owning the home*; commute → *while working*; else *always*) with a **per-line override** in the builder; the projector charges a line only while its condition holds, and the sold variants drop the housing costs. Engine + data-model + builder-UI change. Guard with reconciliation tests (property costs **£0** in post-sale years; commute **£0** from the retirement year; the spend charged each year == the sum of the lines *active* that year — no phantom charge, no silent drop).
   2. **Per-variant deterministic cashflow ladder** — show the year-by-year differences *by housing strategy* (the ladder currently runs the raw household, ignoring the variant transforms). Needs a deterministic per-variant projection; pairs with #1, and also lands the **house-sale milestone** (deferred from the milestones layer).
   3. **Editable-assumptions layer ("everything editable" — Rob's direction).** Make all thresholds/assumptions user-editable in the UI: investment growth, inflation, house/rent growth, the **age of death** (surface the existing per-person longevity lever + show the modelled death year), and the **cost components** (decompose selling costs into estate agent + legal/conveyancing + EPC/removals). Keep the sourced presets (FCA/DMS/OBR) as starting points that derive a user-tweakable **custom set**, with live preview. Patterns + free-tool references: docs/RESEARCH-editable-assumptions-ux.md.
   4. **Buy-vs-rent as a deliberate what-if / Compare** (reuse the delta-child + Compare infrastructure), not baked into every report; + the per-option plain-English **"why"** narrative, milestone-anchored, lint-safe.
   5. **Input-sanity rate/£ builder validation** (a live £-for-a-rate readout + an out-of-range flag) and **real-time cost toggles** (mortgage / service charge / commute on/off, live) — part of, and folded in alongside, the editable-assumptions layer (#3).
