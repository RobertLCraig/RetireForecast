...
**Stage:** active
**Status:** Phase D go-live. The **adviser-legibility workstream** is the active priority (from Rob's 2026-06-29 browser walkthrough). Its legibility **presentation layer is built** (house-sale waterfall, assumptions panel, itemised per-year spend, life-event milestones, input-sanity notes), the **contingent-cost correctness fix (#1, option b) is built** (auto-classified by label; sell variants drop the phantom mortgage, the commute stops at retirement; per-line *override UI* still to build), and now the **per-variant deterministic cashflow ladder (#6) is built** — a strategy selector runs each housing option through `HousingComparison::variantInputs()` (the same single source as the Monte Carlo), so the ladder + milestones *show* the corrected per-strategy numbers and the house-sale milestone landed. The results page also gained a sticky **"on this page" side nav** (browser-verified desktop; mobile deferred). **The next build is the editable-assumptions layer** ("everything user-editable" — Rob's direction), then **buy-vs-rent as a deliberate what-if/Compare**. See What's next + docs/PLAN.md + DECISIONS 2026-06-29 + docs/RESEARCH-editable-assumptions-ux.md.
_Last updated: 2026-06-29 (built **#6 per-variant cashflow ladder** via `deterministicVariants()` + a per-strategy selector on the results page — ladder/milestones follow the selected strategy, house-sale milestone landed, PDF follows the scenario variant, provenance held on the selected variant; plus a sticky **"on this page" nav**, browser-verified desktop. Next: the editable-assumptions layer.)_
