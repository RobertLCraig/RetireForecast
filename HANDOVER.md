...
_Last updated: 2026-07-05 (**decision-support Phase 4 lever menu — three survivor levers built** (two now remain).
Newest: the **per-person longevity** sweep (`Sweep\Lever\PersonLongevityLever` + `Person::withLongevity`) — sets ONE
named person's `LongevityAdjustment` to a ±year `OffsetYears` on that person alone, so a sweep asks "whose longevity
moves the odds?" (the insight the combined bump hides). It is `LeverDirection::Unknown` (non-monotone — extending the
better-provided partner helps, the survivor hurts), but **CRN-safe**: the offset applies *after* the peer death is
drawn, so the mortality draw count is unchanged (resolved the flagged RNG modelling call — it came out safer than the
plan assumed). It is the first **per-person-parameterised** lever: `?string $leverParam` (the person id) threads
through `LeverThresholdService` + `ThresholdRunner` into a new nullable `threshold_results.lever_param` column (joins
the inputs hash — one person's threshold is never served for another; kept separate from `lever_key`), and the
`ThresholdExplorer` menu gains a composite-id entry per person ("How long <person> lives"), gated to a couple. This
foundation is reused by the coming defer-the-survivor's-SP lever. **Two survivor levers remain** (defer-the-
survivor's-SP, care on/off — see What's next). Also this session: a small builder polish — a free-text **"Note"** on
each Other-income row (what it is / where it's from), a **visual aid only** (the assembler never reads it, so it
reaches no engine figure). It follows the two earlier survivor levers — **joint-life-annuity survivor share**
(`SurvivorAnnuityFractionLever`) and **survivor's-DB-share** (`SurvivorDbFractionLever`), both monotone `Increasing` +
CRN-safe, each varying only an *existing* survivor benefit — and the **survivor-cliff story**
(`ResultPresenter::incomeFloor()` survivor-year twin) + **Phase 3** (the combination-comparison surface on Compare).
DECISIONS 2026-07-05. Phases 1 + 2 (the queued `ThresholdResult` backend + the "How far can we go?" panel) are in the
Session log.)_
...
| docs/PLAN-decision-support.md | Staged plan for the "lever thresholds + combination comparison" feature. **Phases 0–3 BUILT + Phase 4 PART-BUILT** (Phase 0 spine 2026-07-04; Phases 1–3 + the Phase-4 survivor-cliff story + three of five survivor levers — DB / annuity / per-person longevity, 2026-07-05). Q2 (family-money × PC) + Q3 (target) resolved. **Remaining:** two Phase-4 survivor levers (defer-the-survivor's-SP, care on/off), then Phase 5 (2-D frontier) + Phase 6 (assistant tie-in). Hardened by a 4-agent review. |
