...
_Last updated: 2026-07-02 (**three silent-drop backlog fixes**: (1) `DbPension::spousePensionFraction` — survivor
keeps `pension × fraction` for life instead of £0 on the member's death; (2) `Person::salaryGrowth` — per-person
real salary-growth override (projector `salaryFactor` now per-person); (3) **DB commutation** —
`commutationLumpSum`/`commutationFactor` (retyped `Percent`→`float`) now reduce the pension by `lumpSum ÷ factor`
and pay the tax-free lump sum at NRA. Completeness tests pin each; DECISIONS 2026-07-02. Rob chose **wire all
three** remaining fields — still to wire: `Person::niCategory`, `Property::ownershipShare`.)_
