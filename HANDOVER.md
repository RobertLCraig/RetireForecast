...
2. **Post-v1 backlog — COMPLETE (Rob approved all my recommendations 2026-06-30; research in [docs/RESEARCH-stress-test-and-official-sources.md](docs/RESEARCH-stress-test-and-official-sources.md)).** All built: **what-if sliders** (b), **retirement-year salary proration** (f), **v2 annuitisation** (c), the **historical stress-test** (a — CC BY-NC-SA JST source, flagged in-code as a public-release blocker), the **ONS mortality refresh + integrity guardrail** (e — `mortality:refresh`), and **care-cost stochasticity** (d — sampled care spell in the Monte Carlo, opt-in, sourced Dilnot/PSSRU/LaingBuisson, care-risk results panel). Low-value hardening leftovers (confirm worth it): tamper-evident run hash, forecast caching.
