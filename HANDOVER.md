...
_Last updated: 2026-07-01 (Lane A built **v2 annuitisation** and the **historical stress-test** (JST Macrohistory total-return data, source flagged as a public-release blocker). Remaining Lane A: ONS-refresh script (ready) + care-cost (sources locked). Lane B shipped the estate-inheritance fix + V2 review polish. Feature work pending Rob's browser sign-off. **⚠️ Concurrent sessions may be live (lanes A/B/C/D) — read the next section before you commit.** Narrative in the session log.)_
...
- **Lane A — post-v1 enhancement backlog (this handover's "What's next #2").** Built + committed: **what-if
  ...
  **historical stress-test** (sequence-of-returns backtest on JST Macrohistory total-return data; engine `5938612`,
  panel `2cad843`). **Remaining Lane A:** the **ONS-refresh script** (fully ONS, ready to build) and **care-cost
  stochasticity** (sources locked: LaingBuisson fees + PSSRU probability/duration + ONS health-state life
  expectancy). Touches `PathProjector` / `HouseholdAssembler` / the builder + results presentation.
...
2. **Post-v1 backlog (Rob approved all my recommendations 2026-06-30; research in [docs/RESEARCH-stress-test-and-official-sources.md](docs/RESEARCH-stress-test-and-official-sources.md)).** Built: **what-if sliders** (b), **retirement-year salary proration** (f), **v2 annuitisation** (c), and the **historical stress-test** (a — sequence-of-returns backtest on JST Macrohistory total-return data; the source is CC BY-NC-SA, flagged in-code as a public-release blocker). **Remaining:** (e) **ONS-refresh script** — **fully ONS, ready to build**: ingest the ONS national + past/projected cohort life tables (xlsx, OGL) and diff against `CohortLifeTable`. (d) **v2 care-cost stochasticity** — **sources locked** (LaingBuisson fees + PSSRU probability/duration + ONS health-state life expectancy for timing; each cited with verified_on); not yet built. Low-value hardening leftovers (confirm worth it): tamper-evident run hash, forecast caching.
