...
_Last updated: 2026-06-29 (added **one-click "quick what-ifs"** — "Retire 2 years later" / "Live 10 years longer" buttons on the base results page + dashboard; each generates an ordinary delta-child via `QuickWhatIf` + `BuilderStateDelta::diff`, then opens it. Earlier this session: **what-ifs highlight what they changed from the base** (`WhatIfChanges` + `BuilderStateDelta::valueAt`) and the **editable-assumptions core** (sparse `assumptionOverrides` delta). All pending Rob's browser sign-off. Next: live preview + the longevity-lever UX.)_
