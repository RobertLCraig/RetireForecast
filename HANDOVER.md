...
_Last updated: 2026-06-29 (built the **editable-assumptions core**: six economic assumptions editable on builder step 1, deriving a custom set from the chosen preset via a sparse `assumptionOverrides` delta; engine `AssumptionSet::with*` + `AssumptionOverrides` applied once in `ScenarioForecaster::assumptions()` so every consumer + the frozen snapshot share it; results panel marks customised figures. Pending Rob's browser sign-off. Next: live preview + the longevity-lever UX.)_
