...
_Last updated: 2026-07-06 (**decision-support Phase 4 — the fourth survivor lever: defer the survivor's State Pension**,
plus a **State Pension deferral correctness fix that had to land first**. The engine modelled deferral as a **free
uplift** (uplifted rate from State Pension age, no forgone income) — a free lunch that would make the lever meaningless
and mislead; Rob chose to **fix the model, then build the lever**. Now deferral **delays the claim**: `PathProjector`
gates the paid pension on a new per-person `spClaimYear = spaYear + round(deferralWeeks/52)` (nothing paid in the
deferral window; uplifted rate from the later start), `spaYear` unchanged (still governs NI + the Pension Credit
qualifying-age gate), a **Pension-Credit notional add-back** so deferring can't conjure PC, and the results-page SP
milestone lands on the claim year. Then the lever: `Sweep\Lever\StatePensionDeferralLever` (per-person, sweeps years →
weeks via `StatePensionEntitlement::withDeferralWeeks`), `LeverDirection::Unknown` (non-monotone) but **CRN-safe** (it
scales a deterministic figure — the flagged RNG call resolved: no desync, like the longevity lever), reusing the
`lever_param` parameterisation; `LeverKey::StatePensionDeferral` + `ThresholdExplorer` menu ("How long <person> defers
their State Pension", one per person who holds a State Pension, gated to a couple). The **headline completeness test now
lands**: deferring the survivor's SP raises the survivor-year floor while deferring the first-dier's does not.
DECISIONS 2026-07-06. **One survivor lever remains** (care-on/off pinned — see What's next).)_
...
| docs/PLAN-decision-support.md | Staged plan for the "lever thresholds + combination comparison" feature. **Phases 0–3 BUILT + Phase 4 PART-BUILT** (Phase 0 spine 2026-07-04; Phases 1–3 + the Phase-4 survivor-cliff story + four of five survivor levers — DB / annuity / per-person longevity, 2026-07-05; defer-the-survivor's-SP on a deferral correctness fix, 2026-07-06). Q2 (family-money × PC) + Q3 (target) resolved. **Remaining:** one Phase-4 survivor lever (care on/off), then Phase 5 (2-D frontier) + Phase 6 (assistant tie-in). Hardened by a 4-agent review. |
