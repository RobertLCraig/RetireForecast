...
_Last updated: 2026-07-01 (Lane A built **v2 annuitisation**, the **historical stress-test** (JST Macrohistory, flagged public-release blocker), and the **ONS mortality-refresh + integrity guardrail** (`mortality:refresh`). Only **care-cost** remains open on Lane A. Lane C shipped withdrawal-sequencing; Lane B's forced-housing workstream is built. Feature work pending Rob's browser sign-off. **⚠️ Concurrent sessions may be live (lanes A/B/C/D) — read the next section before you commit.** Narrative in the session log.)_
...
- **Lane A — post-v1 enhancement backlog (this handover's "What's next #2").** Built + committed: **what-if
  sliders**, **retirement-year salary proration**, **v2 annuitisation** (`7bcbede`/`d85f5bf`), and (2026-07-01) the
  **historical stress-test** (`5938612`/`2cad843`), and the **ONS mortality refresh + integrity guardrail**
  (`mortality:refresh`, `4bc47ea`). **Remaining Lane A:** only **care-cost stochasticity** (sources locked:
  LaingBuisson fees + PSSRU probability/duration + ONS health-state life expectancy). Touches `PathProjector` /
  `HouseholdAssembler` / the builder + results presentation.
...
The adviser-legibility workstream is done (see Current state). What remains is verification and an optional post-v1 backlog.
0. **Forced-housing-event workstream (Lane B — the V2 pressure-test): BUILT + COMMITTED.** (A) means-tested benefits, (B) mortgage-redemption event, (C) feasibility flags, (D) input clarity all landed. **Deferred Lane B items:** stop the bundled mortgage *payment* after a repay-from-capital redemption (a `while_mortgaged` expense condition, designed not built) + the in-place forced-sale model. Detail: DECISIONS + docs/PLAN.md 2026-06-30. Coordinate via the Multi-agent coordination section before touching `PathProjector`.
...
2. **Post-v1 backlog (Rob approved all my recommendations 2026-06-30; research in [docs/RESEARCH-stress-test-and-official-sources.md](docs/RESEARCH-stress-test-and-official-sources.md)).** Built: **what-if sliders** (b), **retirement-year salary proration** (f), **v2 annuitisation** (c), the **historical stress-test** (a — CC BY-NC-SA JST source, flagged in-code as a public-release blocker), and the **ONS mortality refresh + integrity guardrail** (e — `mortality:refresh`; verifies the generated table matches its JSON source + flags staleness + diffs a new release). **Remaining:** (d) **v2 care-cost stochasticity** — **sources locked** (LaingBuisson fees + PSSRU probability/duration + ONS health-state life expectancy for timing; each cited with verified_on); the last open Lane A item. Low-value hardening leftovers (confirm worth it): tamper-evident run hash, forecast caching.
