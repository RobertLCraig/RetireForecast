...
_Last updated: 2026-07-08 (**the assistant generation is now a queued worker turn** — Rob's Compare-page
question died as a raw nginx 504: the synchronous v1 ran the Ollama generation inside one Livewire request
(120s × 2 attempts) past the web server's 60s gateway default. `ask()` now persists a transient encrypted
`AssistantTurn` and the panel polls it like a run (same lifecycle + awaiting-worker hint; the row is deleted
once its answer joins the browser-local transcript; `Gate::forUser` on the turn's owner decides advice mode).
**Assistant answers now need the queue worker running.** Context assembly moved intact to
`App\Assistant\AssistantTurnRunner`; guards G1/G2 unchanged. Alongside it: a per-site Herd nginx conf raises
this site's gateway timeout to 300s (machine config — How to pick up); the question that 504'd asked the
assistant to BUILD a what-if — a capability it deliberately lacks (the approved-but-unbuilt
docs/PLAN-assistant-scenario-editing.md) — so the requested combined what-if was **hand-built as delta-child
"Stay put + child £300/mo + part-time 7/14 + LTM roll-up"** (10k-path run completed, on Compare); and
`php artisan migrate` applied the local DB's **pending Phase-5 frontier migration** (the trade-off map would
have errored in the browser before this). DECISIONS 2026-07-08.)_
_Earlier 2026-07-08: (**housekeeping: the freshness guardrails are now a scheduled CI run** — a new
`data-freshness` GitHub Actions workflow runs `figures:freshness` + `mortality:refresh` monthly + on dispatch,
failing loudly on staleness/drift (both pass today; monthly-not-push rationale in DECISIONS 2026-07-08; it takes
effect on GitHub only once pushed). Agent-side prep for Rob's browser pass also landed: the scripted **Pa11y smoke
re-ran green** with the public `/methodology` page newly added to the sweep (docs/A11Y.md sweep log), and the local
DB's **run-state was verified fresh** — every V2 scenario (base + 14 what-ifs) holds a completed 10,000-path run on
the current engine stamped 2026-07-07, so the earlier "0 or stale runs — re-run first" caveat is obsolete. No
`ThresholdResult`s exist, which is correct: computing a limit / the trade-off map is part of the browser flow, with
the queue worker running.)_
_Earlier 2026-07-07: (**decision-support Phase 6 BUILT — the feature is now COMPLETE (Phases 0–6).** The
assistant now **states** the scenario's computed lever limits (never calculates one): a new
`App\DecisionSupport\ThresholdFacts` turns Done `ThresholdResult`s into `AssistantFact`s **only when the row's
stored `inputs_hash` still matches a hash recomputed against the scenario's CURRENT effective form-state** — the
belt-and-braces the plan's risk note demanded, tested on the exact bypass case (a direct state edit that leaves the
row alive yet mutes it). Facts join `ScenarioContext` (new `$extraFacts` hook), which is both model context and the
G1 allow-list, so fresh = the banded sentence answers grounded and stale = the SAME sentence is refused by
`FigureGrounding` with no new guard code; no fresh limits = an honest "none computed yet" fact. One wording home
per limit (`ThresholdPresenter::meterCaption` now public; `FrontierPresenter` summary + chips; care = the pinned
before/after) and `OutputPhrasing::violations` asserted empty on the fact text. The context also now **volunteers
the survivor cliff** (income-floor facts: both-alive floor + survivor-year twin + the cliff) with new starter
questions. Guards: `ThresholdFactsTest` (the staleness gate on the delete-bypass case, the grounded-then-refused
round trip, frontier/care fact wording, the survivor-cliff facts, neutral phrasing). DECISIONS 2026-07-07 (Phase 6).
**Decision-support remaining:
Rob's browser sign-off only.** Earlier today: **Phase 5 BUILT — the 2-D trade-off map (frontier).** The engine's S2
`frontier` now keeps **every measured cell** (`FrontierPoint` carries its full per-column `SweepCurve` beside the
crossing — the iso-line emerges from the very cells it is derived from, so map and line cannot disagree) and reports
**per-cell progress**; `LeverThresholdService::computeFrontier` (same forecaster + pinned seed as the 1-D compute;
refuses a same-lever pair and the categorical care toggle) + a deliberately **coarse `defaultConditionGrid`** (5 held
values — each column costs a full CRN sweep). The queued run **rides the SAME `ThresholdResult`** record: additive
`condition_lever_key`/`condition_grid` columns are the discriminator (null = 1-D) and join the inputs hash, so
Phase-1's edit-invalidation, live progress, cancel, awaiting-worker hint and owner-scoped CSV route all apply for
free (`FRONTIER_DEFAULT_PATHS` 1,000 paths/cell — Wilson CI ≈±2 points near 90%). UI: a **"trade-off map"** block on
the "How far can we go?" panel, gated to a configured buy + someone still working (the headline buy-price ×
retirement-age pair): a summary sentence that **pins both levers**, per-column banded chips, and an analyst
disclosure with the full tinted-cell table (each cell's % as text, colour never alone) + `FrontierCsvExporter`.
**Correctness pin per the spec:** a frontier column is byte-identical to the Phase-1 1-D compute on a scenario that
actually holds the condition. DECISIONS 2026-07-07 (heatmap-over-curve-family + record-reuse rationale);
docs/PLAN-decision-support.md build record; DATA-MODEL updated. **Decision-support remaining: Phase 6 (assistant
tie-in) + Rob's browser read of the map on the V2 scenario (queue worker running).**)_
_Earlier 2026-07-06: (**built the age-varying spending "smile"** — the promoted "next engine piece". Flat-real
spend understated safe early spend (manufacturing under-spending + biasing the buy/rent verdicts); now a per-line,
piecewise-real `SpendPath` (`{fromAge, amount}` bands) reaches the projection + MC + `HistoricalBacktester`. The
**representation was researched** (Voyant / Kitces-Basu per-category / RightCapital phases / Blanchett curve →
**per-item age-bounded breakpoints**; docs/RESEARCH-under-spending-smile.md) and Rob chose the **per-line-item** scope.
`ExpenseProfile` carries essential/discretionary as `SpendPath`s alongside the headline scalars (the scalar IS the
first band — enforced, one home); the assembler sums each line's bands into the aggregate paths (reconciled: aggregate
== Σ line paths at every age); the projector charges at the reference (first-declared) person's age. A flat plan is
**byte-identical** to the pre-smile engine (suite stayed green with no test edits per slice). Builder **band editor**
(per always-condition spend line, sparse storage) + an **input-sanity note**; ladder/preview/PDF/lever-explorer reflect
it automatically. Guards: `SpendPathTest`, `SpendingSmileTest`, `SpendingSmileAssemblerTest`, `ScenarioBuilderSmileTest`,
`InputNotesTest`. **Committed engine+data layer c120cdb**, then the builder UI + surfacing. v1 limits (flagged): bands
key off person 1's age; only always-lines smile (contingent costs stay flat); the late-life rise is the separate care
spell, not a band. Fast-follows: the hand-draw-the-smile editor + %/yr-and-phase templates; what-if delta highlighting
of band changes. DECISIONS + DATA-MODEL + PLAN 2026-07-06. **Earlier 2026-07-06: validated the engine against James
Shack's "Mark" under-spending case study** — our IHT
...
the **"smile"** (now built, above), a net-new **lifetime-gifting lever**, an under-spender headline
reframe. Earlier 2026-07-06: **equity-release lifetime mortgage now modelled + two V2 what-ifs built.** A new nullable
...
per the plan). So the app **branches** for this lever — no slider / live line / meter / S-curve, and it **ignores the
crossing** (an interpolated "63% of care" limit is meaningless): `ThresholdPresenter::careComparison` renders a two-state
before/after (pictograph + word-band + each state's own Wilson CI + paths), the delta read qualitatively (real only when
the CIs separate). The CSV omits the crossing and names the two states. Offered **ungated** (care applies to a lone
person too). Grounded by a **4-agent understanding→design workflow**; the design's two "decisions" were already settled by
the plan (pin-and-compare + defer substreams). DECISIONS 2026-07-06. **Phase 4 is now done (survivor-cliff story + all 5
levers); next is Phase 5 (2-D frontier) + Phase 6 (assistant tie-in) — see What's next.**)_
_Earlier 2026-07-06: the **fourth** lever, **defer-the-survivor's-SP** (`StatePensionDeferralLever`), on a **State Pension
deferral correctness fix** — the engine had modelled deferral as a **free uplift** (no forgone income); Rob chose to fix
the model first, so deferral now **delays the claim** (`spClaimYear = spaYear + round(deferralWeeks/52)` in `PathProjector`,
forgone income in the window, a Pension-Credit notional add-back, the SP milestone on the claim year). The headline test
lands: deferring the survivor's SP raises the survivor-year floor while the first-dier's does not. `LeverDirection::Unknown`
but CRN-safe. DECISIONS 2026-07-06; detail in the Session log._
_Earlier 2026-07-05: the **per-person longevity** sweep (`PersonLongevityLever`, the first per-person-parameterised
lever — `lever_param` column + composite menu id), the two survivor-share levers (**DB** + **joint-life annuity**), the
**survivor-cliff story** (`ResultPresenter::incomeFloor()` survivor-year twin) + **Phase 3** (combination comparison on
Compare), and Phases 1 + 2 (the queued `ThresholdResult` backend + the "How far can we go?" panel). DECISIONS 2026-07-05;
detail in the Session log._
...
**built the whole in-app local-model assistant** (docs/RESEARCH-local-assistant.md): a docked side-panel "chatbot"
on the results + Compare pages, now **complete across its three specced phases** — (1) a grounded scenario/comparison
explainer, (2) methodology doc-RAG (a **curated** corpus + the new **`/methodology` page**/doc, hand-rolled cosine,
`assistant:index-docs`; live verification forced the curation — LA-9), and (3) **idea capture** (the "Ideas" tab, the
model's only write: an append-only attributed backlog it never builds from). All app-layer, engine untouched, each
phase verified end-to-end against the real `qwen3:14b`.
...
- **In progress:** nothing mid-edit. One live carry-over a fresh agent must know: the real **V2 couple's data** is captured privately in the gitignored `docs/SCENARIO-V2.local.md` (never commit) — the durable source to rebuild the scenario after a DB wipe. (The earlier stale-runs carry-over is resolved: verified 2026-07-08 that every V2 scenario holds a completed current-engine run stamped 2026-07-07.)
