...
_Last updated: 2026-07-02 (**two silent-drop backlog fixes**: (1) `DbPension::spousePensionFraction` now paid —
the survivor keeps `pension × fraction` for life instead of £0 on the member's death; (2) `Person::salaryGrowth`
now consumed — a per-person real salary-growth override (projector `salaryFactor` is now per-person), falling
back to the assumption set. Completeness tests pin both; DECISIONS 2026-07-02. Next silent-drop fixes:
`DbPension` commutation, `Property::ownershipShare`, `Person::niCategory`.)_
